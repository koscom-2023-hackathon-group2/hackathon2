...
public class InviteController {
   ...
   /* @GetMapping(value = "/invite", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    ...
    // get invite requests.
    @GetMapping("/invite")
    public ResponseEntity<List<Invite>> sendInviteRequests(@RequestParam("host")String hostId){
    ...
    public ResponseEntity<Void> invite_agree(@RequestBody InviteDto inviteDto){
        ...
        // if get response of the invite, then delete it from the waiting invite list.
        inviteRepository.delete(inviteRepository.findByInviteeIdAndAccount(inviteDto.getInvitee(), inviteDto.getAccount()));
        return ResponseEntity.ok().build();
    }
}
